...
{
    class DocNotaCredito : FormCommon, IForm
    {
        ...
        public DocNotaCredito(Dictionary<string, IForm> dictionary)
        {
            mForm = createForm(Conexion.company, Conexion.application, AddonSeidorMobile.Properties.Resources.frmNotaCredito, FormName.DOC_NOTA_CREDITO);
            if (mForm != null)
            {
                ...
                StatusMessageError("Constructor() > No se pudo crear el formulario " + FormName.DOC_NOTA_CREDITO + " revise el log del sistema.");
        }
            ...
            try
            {
                    ...
                    itemEvent.ItemUID.Equals(GRID_PRINCIPAL))
                {
                    ...
                    if (itemEvent.BeforeAction)
                    {
                        ...
                        ((SAPbouiCOM.EditTextColumn)mGrid.Columns.Item(16)).LinkedObjectType = Constantes.OBJ_TYPE_NOTA_CREDITO;
                }
                else {
