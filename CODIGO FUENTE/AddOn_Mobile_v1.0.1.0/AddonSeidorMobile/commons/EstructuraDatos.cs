...
namespace AddonSeidorMobile.commons
{
    public class EstructuraDatos : FormCommon
    {
        ...
        private void InsertarDatosBdMobile()
        {
            foreach (var item in data_schema.database.BdMobile.getInserts())
            {
                makeInsert(item);
            }
        }
        ...
        private void makeInsert(string query)
        {
            SAPbobsCOM.Recordset rs = Conexion.company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
            ...
            {
                StatusMessageError("makeInsert() > " + ex.Message);
            }
            finally
            {
                if (rs != null)
